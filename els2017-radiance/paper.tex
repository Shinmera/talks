...
\begin{abstract}
  Radiance\cite{radiance} is a set of libraries that provide an environment for web applications. By putting its focus on running multiple web services or applications within the same environment, it has developed a features that set it apart from traditional web frameworks. In particular, it shows a new approach to the way different framework features are provided, and how the routing to content is performed. These differences allow applications written against Radiance to transparently share common resources such as the HTTP server, user accounts, the authentication, and so forth with each other.
\end{abstract}
...
Thanks to Joram Shrijver, Till Ehrengruber, and Fernando Borretti for feedback and suggestions on the paper.
