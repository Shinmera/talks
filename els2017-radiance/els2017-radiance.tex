...
As an example, let us imagine a server set up on \\ \mintinline{html}{radiance.example.com/test/} that runs a single application. The application will be called ``hello'' and has the following code to define a page:
...
If an (untranslated) request to \mintinline{html}{hello/index} is performed, the above code will be executed, and the template system Clip will render the \mintinline{html}{index.ctml} file. The template might look as follows:
...
Clip will recognise the \mintinline{html}{@href} attribute, parse its value as a URI, send it through the reversal routes, and set the resulting URL as the value for the \mintinline{html}{href} attribute. Thus the cycle illustrated in Figure 1 is followed by our minimal setup here. \\

As it stands, Radiance would translate a request to \\ \mintinline{html}{http://radiance.example.com/test/index} into the URI \\\mintinline{html}{radiance/test/index}, which is not what we need if the index page should be invoked. Routes need to be set up to handle the conversion to and from the internal representation.
...
With these routes in place, our previous example URL will be translated into \mintinline{html}{hello/index} as required. Furthermore, the requested reversal translation of the URI \\\mintinline{html}{/static/hello/index.css} will be turned into the URL \\\mintinline{html}{http://radiance.example.com/test/static/hello/index.css}, completing the cycle properly. \\

While the above routes are fairly simplistic in their behaviour, routes can be written to perform arbitrary operations, including stashing away data in the request object for later use during reversal. This allows things like the virtual subdomain route system that is included by default. This route recognises when a request is made on the path prefix \mintinline{html}{/!/subdomain/} and translates it into a request with the subdomains set to the subdomain name in the prefix and the path stripped of this prefix. It is necessary for the reversal routes to know that the initial request came through this particular route, in order to appropriately translate back to the prefix, as there is no indication in the actual URI that this should happen. \\
