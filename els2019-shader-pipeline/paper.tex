...
Objects are typically rendered using the \code{render} function. Object classes are encouraged to add methods that specialise the behaviour and perform necessary setup, as well as to perform the final draw call. For instance, a very primitive class could look as shown in \autoref{lst:simple-object}.
...
\code{render-with} thus exists mostly as an entry point for rendering operations to allow shader passes greater control over the rendering. Typically it will simply defer to \code{render}. This inversion is very important for shader passes that take all control away from the objects. Without it, the \code{:before}, \code{:after}, and \code{:around} methods on \code{render} specialised on an object could try to manipulate state before methods specialised on a shader pass could prevent it. \\
...
Shader passes not only retain information about how objects are drawn with it, but also about the input and output textures that a pass interacts with. For this reason, each shader pass is also a node in a graph, with distinct input and output ports.
