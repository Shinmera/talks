...
\code{render-with} thus exists mostly as an entry point for rendering operations to allow shader passes greater control over the rendering. Typically it will simply defer to \code{render}. This inversion is very important for shader passes that take all control away from the objects. Without \code{render-with}, the \code{:before}, \code{:after}, and \code{:around} methods on \code{render} specialised on an object could try to manipulate state before methods specialised on a shader pass could prevent it. \\
